...
WATCH_EXTENSIONS = {".py"}
HOT_RELOAD_ENV_VAR = "MASTER_APP_HOT_RELOAD_ACTIVE"
_LOCK_HANDLE = None
...
def _iter_watch_files(roots: Sequence[str]) -> Iterable[str]:
    for root in roots:
...
def _snapshot(roots: Sequence[str]) -> Dict[str, Tuple[int, int]]:
    out: Dict[str, Tuple[int, int]] = {}
    for path in _iter_watch_files(roots):
        try:
            stat = os.stat(path)
        except OSError:
            continue
        out[os.path.normpath(path)] = (int(stat.st_mtime_ns), int(stat.st_size))
    return out
...
def _display_path(path: str, roots: Sequence[str]) -> str:
    ...
    for root in roots:
        ...
        try:
            ...
            continue
        label = os.path.basename(normalized_root)
        try:
            rel_path = os.path.relpath(normalized_path, normalized_root)
        except ValueError:
            rel_path = normalized_path
        return os.path.join(label, rel_path)
...
def _write_reload_request(
    ...
    payload = {
        ...
        "files": [_display_path(path, roots) for path in changed_paths[:20]],
    }
...
def _acquire_single_instance_lock(root: str):
    try:
        ...
        name = f"Global\\MASTER_APP_HOT_{key}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dev hot-restart launcher for Master App.")
    ...
    explorer_root = os.path.join(os.path.dirname(root), "truck_nest_explorer")
    watch_roots = [root]
    if os.path.isdir(explorer_root):
        watch_roots.append(explorer_root)
    ...
    print("Master App hot restart launcher running.")
    ...
    print(f"Watching: {', '.join(watch_roots)}")
    ...
    try:
        while True:
            ...
            if changed:
                ...
                short = [_display_path(path, watch_roots) for path in changed[:4]]
